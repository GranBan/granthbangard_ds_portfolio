...
graph LR
    subgraph Row1[ ]
        direction LR
        A[Cricsheet JSON] --> B[Parser] --> C[Ball-by-ball dataframe] --> D[2nd innings filter] --> E[Over-level aggregation] --> F[Feature engineering]
    end
    subgraph Row2[ ]
        direction LR
        G[Temporal split] --> H[Logistic Regression baseline] --> I[XGBoost] --> J[Isotonic calibration] --> K[SHAP feature selection] --> L[Streamlit deployment]
    end
    F --> G
```
